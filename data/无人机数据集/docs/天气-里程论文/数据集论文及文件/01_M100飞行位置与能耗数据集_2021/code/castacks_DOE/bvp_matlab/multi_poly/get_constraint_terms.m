function [A, b] = get_constraint_terms(q0, params)
    ...
    for i = 1:length(s_vals)
        ...
        for j = 1:params.num_poly_params
            A_curv_1(i, j) = s^j;
            A_curv_1(i+params.num_constraint_samples, j) = -s^j;
        end
    end
    ...
    for i = 1:length(s_vals)
        s = s_vals(i);
        for j = 1:params.num_poly_params
            A_curv_3(i, j+params.num_poly_params) = s^j;
            A_curv_3(i+params.num_constraint_samples, j+params.num_poly_params) = -s^j;
        end
    end
    ...
    for i = 1:length(s_vals)
        s = s_vals(i);
        for j = 1:params.num_poly_params
            A_curv_rate_1(i, j) = j * s^(j-1);
            A_curv_rate_1(i+params.num_constraint_samples, j) = -j * s^(j-1);
        end
    end
    ...
    for i = 1:length(s_vals)
        s = s_vals(i);
        for j = 1:params.num_poly_params
            A_curv_rate_3(i, j+params.num_poly_params) = j * s^(j-1);
            A_curv_rate_3(i+params.num_constraint_samples, j+params.num_poly_params) = -j * s^(j-1);
        end
    end

...
% end
% pow_cpp = dlmread('~/dump/kite/cpp/pow.txt');
% pow_diff = abs(pow_terms - pow_cpp);
% figure, hold on;
% plot(1:size(pow_diff, 1), pow_diff(:, 1), 'r-');
% plot(1:size(pow_diff, 1), pow_diff(:, 2), 'b-');

% check simpson weights
% weights = [1, 4, repmat([2, 4], 1, (101-3)/2), 1];
% wts_cpp = dlmread('~/dump/kite/cpp/weights.txt');
% wt_diff = abs(weights' - wts_cpp);
% figure, plot(wt_diff);
...
% grad_curv = grad_curv';
% A_cpp = dlmread('~/dump/kite/cpp/A_ineq.txt');
% grad_ineq_cpp = dlmread('~/dump/kite/cpp/grad_ineq.txt');
% diff_A = abs(A_curv - A_cpp);
% diff_grad = abs(grad_curv(:) - grad_ineq_cpp);
% figure, plot(diff_A);
% figure, plot(diff_grad); 
...
% end
% c1_cos_cpp = dlmread('~/dump/kite/cpp/c1_cos_cache.txt');
% c2_cos_cpp = dlmread('~/dump/kite/cpp/c2_cos_cache.txt');
% c3_cos_cpp = dlmread('~/dump/kite/cpp/c3_cos_cache.txt');
% c1_sin_cpp = dlmread('~/dump/kite/cpp/c1_sin_cache.txt');
% c2_sin_cpp = dlmread('~/dump/kite/cpp/c2_sin_cache.txt');
% c3_sin_cpp = dlmread('~/dump/kite/cpp/c3_sin_cache.txt');
% diff_c1_cos = abs(c1_cos_cache - c1_cos_cpp);
% diff_c3_cos = abs(c3_cos_cache - c3_cos_cpp);
% diff_c2_cos = abs(c2_cos_cache(1:2) - c2_cos_cpp);
% diff_c1_sin = abs(c1_sin_cache - c1_sin_cpp);
% diff_c3_sin = abs(c3_sin_cache - c3_sin_cpp);
% diff_c2_sin = abs(c2_sin_cache(1:2) - c2_sin_cpp);
% figure, plot(diff_c1_sin);

% check terminal error
% err = terminal_error_sincos(q, params.num_poly_params, X0, Xf, v, vw_x, vw_y, params);
% err_cpp = dlmread('~/dump/kite/cpp/terminal_error.txt');
% diff_err = abs(err - err_cpp);
% figure, plot(diff_err)

% check constraint terms
[constr, ~, grad_constr, ~] = nonlcon_with_grad_sincos(q, X0, Xf, v, vw_x, vw_y, params);
constr_cpp = dlmread('~/dump/kite/cpp/constr.txt');
constr_grad_cpp = dlmread('~/dump/kite/cpp/constr_grad.txt');
diff_constr = abs(constr - constr_cpp);
diff_constr_grad = abs(grad_constr(:) - constr_grad_cpp);
figure, plot(diff_constr)
figure, plot(diff_constr_grad(1:20));
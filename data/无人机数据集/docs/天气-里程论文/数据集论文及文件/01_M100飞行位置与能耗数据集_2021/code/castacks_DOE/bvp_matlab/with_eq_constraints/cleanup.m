...
%                     80, 80, 0, 0;
%                     70, 70, 0, 0];
possible_values = readmatrix('data/large_data.csv');

count = size(possible_values,1);
% save -ascii -double
parfor X=1:count
    tic
    Xf = possible_values(X,1:4);
    ...
    % optimize!
     ...
     while inner_loop_count < 2
         if inner_loop_count == 0
             [q, perf] = solve_bvp_fmincon(X0, Xf, Xf_orig, v, vw_x, vw_y, params,possible_values(X,12:22)','sqp');
         else
             disp('sqp');
             [q, perf] = solve_bvp_fmincon(X0, Xf, Xf_orig, v, vw_x, vw_y, params,possible_values(X,12:22)','interior-point');
         end
...
%         disp("Done with" + mat2str(Xf(1:3)));
        disp(num2str(sum(possible_values(X,20:22))-sum(q(9:11))));

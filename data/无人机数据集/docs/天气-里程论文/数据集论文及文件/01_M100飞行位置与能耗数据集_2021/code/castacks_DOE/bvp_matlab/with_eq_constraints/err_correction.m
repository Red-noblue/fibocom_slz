...
%                     80, 80, 0, 0;
%                     70, 70, 0, 0];
% possible_values = readmatrix('err_.csv');
...
%         disp("have it");
    ...
    inner_loop_count = 0;
    % optimize!
    ind = find(abs(data(:,1)-Xf(1))<=15 & abs(data(:,2)-Xf(2))<=15 & abs(data(:,3)-Xf(3))<= deg2rad(60));
    while inner_loop_count < 1+length(ind)

        if inner_loop_count == 0
            [q, perf] = solve_bvp_fmincon(X0, Xf, Xf_orig, v, vw_x, vw_y, params);
        elseif inner_loop_count == 1
            disp("sqp");
            [q, perf] = solve_bvp_fmincon(X0, Xf, Xf_orig, v, vw_x, vw_y, params, 1,'sqp');
        else 
            disp("sqp failed, using nearest solution" + mat2str(inner_loop_count-1));
            
%             index = ind(randperm(length(ind)));
            [q, perf] = solve_bvp_fmincon(X0, Xf, Xf_orig, v, vw_x, vw_y, params, data(ind(inner_loop_count-1),12:22)');
            
        end
        tol = constr_violation(q, params.num_poly_params, X0, Xf, v, vw_x, vw_y, params);
        if all(abs(tol) < [0.5, 0.5, 0.1, 0.1]')
            break
        end
    ...
    elseif all(abs(tol) < [0.5, 0.5, 0.1, 0.1]')

function [A, Aeq, grad_A, grad_Aeq] = nonlcon_with_grad_sincos(q, X0, Xf, v, vw_x, vw_y, params)
    ...
    % get curv ineq constraints
    [A_curv, grad_curv] = get_curv_ineq_terms(q, params);
    
    % set up inequality constraints
    err = terminal_error_sincos(q, num_poly_params, X0, Xf, v, vw_x, vw_y, params);
    A = [0.5 * err.^2; 0.5 * c1_df^2] - [params.bv_tol_sincos; params.curv1_tol];
    A = [A; A_curv];
    
    % no equality constraint for now
    Aeq = [];
    
    if nargout > 2
        % set up gradient for ineq constraints
        grad_A = [...
            err(1) * get_grad_x(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            err(2) * get_grad_y(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            err(3) * (-sin_f) * get_grad_psi(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            err(4) * (cos_f) * get_grad_psi(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            err(5) * get_grad_curv(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            get_grad_curv_deriv_cost(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, params.scales); ...
            grad_curv ...
        ]';
...
function [grad_x] = get_grad_x(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    ...
    for i = 2:num_poly_params+1
        % for x1 w.r.t c1
        grad_x(i-1) = grad_x(i-1) - simpson_integration(fun_sinns1, i, 101, 0, sf1) / (i * scales(i-1));
        
        % for x2 w.r.t c1
        grad_x(i-1) = grad_x(i-1) + -(sf1^i * dx2q1_1 / i + sf1^(i-1) * dx2q1_2) / scales(i-1);
        
        % for x3 w.r.t c1
        grad_x(i-1) = grad_x(i-1) - ((sf1^i/i + sf1^(i-1)*sf2) * dx3q1_1 + sf1^(i-1) * dx3q1_2) / scales(i-1);
        % for x3 w.r.t c3
        idx = num_poly_params + i - 1;
        grad_x(idx) = grad_x(idx) - simpson_integration(fun_sinns3, i, 101, 0, sf3) / (i * scales(i-1));
    end
...
function [grad_y] = get_grad_y(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    num_poly_params = length(c1) - 1;
    ...
    for i = 2:num_poly_params+1
        % for y1 w.r.t c1
        grad_y(i-1) = grad_y(i-1) + simpson_integration(fun_cosns1, i, 101, 0, sf1) / (i * scales(i-1));
        
        % for y2 w.r.t c1
        grad_y(i-1) = grad_y(i-1) + (sf1^i * dx2q1_1 / i + sf1^(i-1) * dx2q1_2) / scales(i-1);
        
        % for y3 w.r.t c1
        grad_y(i-1) = grad_y(i-1) + ((sf1^i/i + sf1^(i-1)*sf2) * dx3q1_1 + sf1^(i-1) * dx3q1_2) / scales(i-1);
        % for y3 w.r.t c3
        idx = num_poly_params + i - 1;
        grad_y(idx) = grad_y(idx) + simpson_integration(fun_cosns3, i, 101, 0, sf3) / (i * scales(i-1));
    end
...
function [grad_psi] = get_grad_psi(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    num_poly_params = length(c1) - 1;
    ...
    for i = 2:num_poly_params+1
        % for psi w.r.t c1
        grad_psi(i-1) = grad_psi(i-1) + (sf1^i/i + sf1^(i-1) * (sf2 + sf3)) / scales(i-1);
        
        % for psi w.r.t c3
        idx = num_poly_params + i - 1;
        grad_psi(idx) = grad_psi(idx) + sf3^i / (i * scales(i-1));
    end
...
function [grad_curv] = get_grad_curv(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    num_poly_params = length(c1) - 1;
    ...
    for i = 1:num_poly_params
        % for kappa w.r.t c1
        grad_curv(i) = grad_curv(i) + sf1^i / scales(i);
        
        % for kappa w.r.t c3
        idx = num_poly_params + i;
        grad_curv(idx) = grad_curv(idx) + sf3^i / scales(i);
    end
...
function [grad_curv_cost] = get_grad_curv_cost(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    num_poly_params = length(c1) - 1;
    ...
    for i = 1:num_poly_params
        grad_curv_cost(i) = grad_curv_cost(i) + sf1^i / scales(i);
    end
...
function [grad_curv_cost] = get_grad_curv_deriv_cost(q, c1, c2, c3, psi1, psi2, psi3, sf1, sf2, sf3, v, vw_x, vw_y, scales)
    num_poly_params = length(c1) - 1;

    c1_df = polyval(polyder(c1), sf1);
    c1_ddf = polyval(polyder(polyder(c1)), sf1);
    
    grad_curv_cost = zeros(1, length(q)); % only depends on q1
    for i = 1:num_poly_params
        grad_curv_cost(i) = grad_curv_cost(i) + i * sf1^(i-1) / scales(i);
    end
    
    % w.r.t sf1
    grad_curv_cost(end-2) = grad_curv_cost(end-2) + c1_ddf;
    
    grad_curv_cost = grad_curv_cost * c1_df;
end
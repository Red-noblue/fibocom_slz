# 中文说明：本脚本用于汇总物理模型、补充模型与 R-XGBoost 模型的飞行级绝对相对误差，并兼容当前 R 训练输出目录。
"""
...
def _read_prediction_csv(filename):
    candidates = [
        BASE_DIR / "data" / filename,
        BASE_DIR / "R-XGBoost-Model" / "predicted-values" / filename,
    ]
    for path in candidates:
        if path.exists():
            return pd.read_csv(path, low_memory=False)
    raise FileNotFoundError(f"未找到预测结果文件: {filename}")


def calculate_are_ML():
    cruise = _read_prediction_csv("predictions_cruise.csv")
    landing = _read_prediction_csv("predictions_landing.csv")
    takeoff = _read_prediction_csv("predictions_takeOff.csv")
    data = pd.read_csv(BASE_DIR / "data" / "flights_processed.csv", low_memory=False)
    data['p'] = data.battery_current*data.battery_voltage
    sample = pd.read_csv(BASE_DIR / "data" / "sample.csv")
...
def main():
    ...
    coeff = pd.read_csv(BASE_DIR / "data" / "coefficients.csv")
    sample = pd.read_csv(BASE_DIR / "data" / "sample.csv")
    summary = pd.read_csv(BASE_DIR / "data" / "energy_summary.csv")
    df = summary[~summary.flight.isin(sample.flight)]
    df.reset_index(inplace=True)
    model_2_error = pd.read_csv(BASE_DIR / "data" / "model_2error.csv")
    ...
    plt.savefig(BASE_DIR / "results" / "figureS12.pdf")
    plt.show()

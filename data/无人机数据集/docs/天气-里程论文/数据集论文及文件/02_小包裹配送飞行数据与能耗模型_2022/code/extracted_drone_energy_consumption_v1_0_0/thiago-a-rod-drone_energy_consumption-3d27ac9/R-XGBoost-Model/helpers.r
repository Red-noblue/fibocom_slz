# 中文说明：本文件提供 R 版 XGBoost 训练辅助函数，并补充模型文件与特征重要性落盘能力。
library(plyr)
library(dplyr)
library(ggplot2)
library(reshape2)
...
hyperpar_tuning_energy_model <- function(df_train_regime, regime_name = ""){
  ...
  plot_dir <- file.path(modelR_dir, "plotsModels", regime_name)
  ...
  for(idx in 1:nrow(hyperpars)){ 
    ...
    mod <- xgb.cv(params = params, 
                  ...
                  custom_metric = relative_mae,  ## our error function
                  maximize = FALSE, 
  ...
  for(i in 1:nrow(hyperpars)){
    ...
    filename <- file.path(plot_dir, paste("hyperpar", i, ".png", sep = ""))
...
train_test_energy_model <- function(df_train_regime, df_test_regime, params, nrounds, regime_name = ""){
  feature_names <- colnames(df_train_regime %>% dplyr::select(-y, -flight, -time))
  model_dir <- file.path(modelR_dir, "models")
  ...
  dtrain <- xgb.DMatrix(as.matrix(df_train_regime %>% dplyr::select(-y, -flight, -time)), 
                        label = as.vector(df_train_regime$y))
  ## train final model
  mod <- xgb.train(params = final_params, 
                   nrounds = nrounds,
                   data = dtrain, 
                   custom_metric = relative_mae, 
                   maximize = FALSE, 
                   verbose = TRUE)

  model_path <- file.path(model_dir, paste("xgboost_", regime_name, ".json", sep = ""))
  xgb.save(mod, model_path)

  importance <- xgb.importance(feature_names = feature_names, model = mod)
  write.csv(importance,
            file.path(model_dir, paste("feature_importance_", regime_name, ".csv", sep = "")),
            row.names = FALSE)

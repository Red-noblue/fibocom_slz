# 中文说明：本文件用于生成车辆对比相关图表，并兼容新版本 matplotlib 的坐标轴字体设置方式。
"""
...
def figure5(df):
    ...
    ax1.barh(df1['Vehicle Class'], df1.ghg_km_fuel, color='lightblue', left=df1.ghg_km_battery + df1.ghg_km_upstream,
             ...
             xerr=np.vstack([np.abs(df1.ghg_error_low), np.abs(df1.ghg_error_high)]), capsize=5)
    ...
    ax2.barh(df1['Vehicle Class'], df1.ghg_fuel_pack, color='lightblue', left=df1.ghg_battery_pack + df1.ghg_upstream_pack,
             label='Fuel consumption',
             xerr=np.vstack([np.abs(df1.ghg_error_low_pack), np.abs(df1.ghg_error_high_pack)]), capsize=5)

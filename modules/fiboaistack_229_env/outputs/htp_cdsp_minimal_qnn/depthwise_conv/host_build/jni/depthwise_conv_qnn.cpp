...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for depthwise_conv_qnn*/
  QnnModel depthwise_conv_qnn;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("depthwise_conv_qnn", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(depthwise_conv_qnn.initialize(backendHandle, interface, contextHandle, "depthwise_conv_qnn", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_input(depthwise_conv_qnn), err);
  VALIDATE(addTensor_conv_weight(depthwise_conv_qnn), err);
  VALIDATE(addTensor_conv_bias(depthwise_conv_qnn), err);
  VALIDATE(addNode__conv_Conv(depthwise_conv_qnn), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&depthwise_conv_qnn};

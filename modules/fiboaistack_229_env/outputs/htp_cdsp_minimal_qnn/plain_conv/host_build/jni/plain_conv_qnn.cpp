...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for plain_conv_qnn*/
  QnnModel plain_conv_qnn;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("plain_conv_qnn", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(plain_conv_qnn.initialize(backendHandle, interface, contextHandle, "plain_conv_qnn", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_input(plain_conv_qnn), err);
  VALIDATE(addTensor_conv_weight(plain_conv_qnn), err);
  VALIDATE(addTensor_conv_bias(plain_conv_qnn), err);
  VALIDATE(addNode__conv_Conv(plain_conv_qnn), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&plain_conv_qnn};

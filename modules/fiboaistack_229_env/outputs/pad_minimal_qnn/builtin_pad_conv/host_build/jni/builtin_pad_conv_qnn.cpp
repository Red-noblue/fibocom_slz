...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for builtin_pad_conv_qnn*/
  QnnModel builtin_pad_conv_qnn;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("builtin_pad_conv_qnn", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(builtin_pad_conv_qnn.initialize(backendHandle, interface, contextHandle, "builtin_pad_conv_qnn", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_input(builtin_pad_conv_qnn), err);
  VALIDATE(addTensor_conv_weight(builtin_pad_conv_qnn), err);
  VALIDATE(addTensor_conv_bias(builtin_pad_conv_qnn), err);
  VALIDATE(addNode__conv_Conv(builtin_pad_conv_qnn), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&builtin_pad_conv_qnn};

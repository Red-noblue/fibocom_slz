...
tools/codex-project-portable/mycodex/sessions/skills/autonomous-skill/scripts/run-session.sh \
  --max-sessions 8 \
  "接入案例2数据并建立 baseline 复现任务清单"

# Continue an existing task
tools/codex-project-portable/mycodex/sessions/skills/autonomous-skill/scripts/run-session.sh \
  --task-name case2-baseline \
  --continue

# List all tasks and their progress
tools/codex-project-portable/mycodex/sessions/skills/autonomous-skill/scripts/run-session.sh --list

# Show help
tools/codex-project-portable/mycodex/sessions/skills/autonomous-skill/scripts/run-session.sh --help
```

The runner intentionally leaves `--model` unset so Codex uses the active `config.toml` or selected profile model by default.

## DeepOFDM Project Defaults

This installed copy is adapted for `DeepOFDM-ReceiverLab`:

- Prefer the project-portable `CODEX_HOME` under `tools/codex-project-portable/mycodex/sessions`.
- Store autonomous task state under project-root `.autonomous/`.
- Default to `workspace-write` and `approval_policy=never`.
- Default to `8` sessions per run, configurable with `--max-sessions` or `AUTONOMOUS_MAX_SESSIONS`.
- Write `nightly_report.md` and `last_message.md` in each task directory.
- Do not commit automatically during unattended work unless the explicit task asks for a commit.
- Use `--network` only for isolated research or dependency tasks where unrestricted shell access is acceptable.

#!/bin/bash
#
# 中文注释：项目化的 Codex 夜间自动续跑脚本，默认使用项目内 portable CODEX_HOME。
#
...
# Configuration
AUTO_CONTINUE_DELAY="${AUTONOMOUS_AUTO_CONTINUE_DELAY:-3}"
DEFAULT_MAX_SESSIONS="${AUTONOMOUS_MAX_SESSIONS:-8}"
CURRENT_TASK_NAME=""
CODEX_ARGS=()
...
# Prefer the project-portable Codex home that contains this skill.
if [ -z "${CODEX_HOME:-}" ]; then
    export CODEX_HOME="$(cd "$SKILL_DIR/../.." && pwd)"
fi

# Resolve project root from the caller's repository. Fall back to current directory.
PROJECT_ROOT="${AUTONOMOUS_PROJECT_ROOT:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"

# Task directory base (in project root)
AUTONOMOUS_DIR="${AUTONOMOUS_DIR:-$PROJECT_ROOT/.autonomous}"
...
handle_interrupt() {
    ...
    if [ -n "${CURRENT_TASK_NAME:-}" ]; then
        write_run_report "$AUTONOMOUS_DIR/$CURRENT_TASK_NAME" "interrupted"
...
show_help() {
    ...
    echo ""
    echo "Defaults:"
    echo "  max sessions: $DEFAULT_MAX_SESSIONS (override with --max-sessions or AUTONOMOUS_MAX_SESSIONS)"
...
# Write a compact report that the human can read the next morning.
write_run_report() {
    local task_dir="$1"
    local reason="$2"
    local report="$task_dir/nightly_report.md"

    mkdir -p "$task_dir"
    {
        echo "# Nightly Report: $(basename "$task_dir")"
        echo ""
        ...
        if [ -f "$task_dir/task_list.md" ]; then
            grep '^- \[ \]' "$task_dir/task_list.md" 2>/dev/null | head -30 || true
        else
            echo "- task_list.md not found"
        fi
        echo ""
        echo "## Recent Progress Notes"
        if [ -f "$task_dir/progress.md" ]; then
            tail -80 "$task_dir/progress.md"
        else
            echo "- progress.md not found"
        fi
        echo ""
        echo "## Last Agent Message"
        if [ -f "$task_dir/last_message.md" ]; then
            cat "$task_dir/last_message.md"
        else
            echo "- last_message.md not found"
        fi
    } > "$report"

    print_info "Nightly report written: $report"
}
...
run_initializer() {
    ...
    codex exec "${CODEX_ARGS[@]}" -o "$task_dir/last_message.md" "Task: $task_desc
Task Name: $task_name
...
$exec_prompt"
    ...
    if [ "$resume_last" = true ] && [ -f "$task_dir/session.id" ]; then
        ...
        codex exec resume "${CODEX_ARGS[@]}" -o "$task_dir/last_message.md" "$session_id" "$prompt" 2>&1 | tee -a "$task_dir/session.log"
    else
        # Start a new session
        codex exec "${CODEX_ARGS[@]}" -o "$task_dir/last_message.md" "$prompt" 2>&1 | tee -a "$task_dir/session.log"
...
main() {
    ...
    local max_sessions="$DEFAULT_MAX_SESSIONS"
    local session_num=1
    ...
    while true; do
        ...
        if is_complete "$task_dir"; then
            ...
            echo ""
            echo "Task directory: $task_dir"
            write_run_report "$task_dir" "all tasks completed"
        ...
        if [ $max_sessions -gt 0 ] && [ $session_num -ge $max_sessions ]; then
            ...
            write_run_report "$task_dir" "max sessions reached"
            exit 0
        fi

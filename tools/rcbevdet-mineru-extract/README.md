...
```bash
cd /home/chenzy/Yansongs_Mickey_Mouse_Clubhouse/哆啦A梦百宝箱/rcbevdet-mineru-extract
...
- `quota` 接口返回 `user_left_quota=0`、`total_left_quota=0` 时，不一定代表 `extract` 已经不可用。
- 在 `2026-05-07` 的本地实测里，`quota` 返回 `0`，但对本地 PDF 执行单页抽取仍然成功完成。
- 因此，若你怀疑配额接口状态异常，先用一个小输入做最小化真实验证，不要只凭 `quota` 输出就中止排查。

建议的最小化验证方式：

```bash
./bin/rcbevdet-mineru-extract extract \
  --out-dir /tmp/mineru_probe \
  --page-ranges 1 \
  /path/to/a.pdf
```
